...
//Autoload el usuario asociado a :userId
	...
	models.User.findById(userId)
		.then(function(user) {
			if (user){
				...
				req.flash('error', 'No existe el ususario con id=' + id + '.');
			...
			if (existing_user){
				...
				return user.save({fields: ["username", "password", "salt"]})
					.then(function(user){
						...
						res.redirect('/session');	//redireccion a la pagina de login
					})
...
exports.update = function(req, res, next){
	...
	if (! req.body.user.password){
		req.flash('error', 'El campo de password debe rellenarse');
